...
namespace DocumentManager.Core.Converters.Handlers
{
    public class DocXHandler
    {
        ...
        public MemoryStream ReplaceMergeField()
        {
        ...
        public MemoryStream ReplaceTexts()
        {
            ...
            using (var doc = WordprocessingDocument.Open(_docxMs, true))
            {
                ...
                foreach (var text in documentTexts.Concat(headerTexts).Concat(footerTexts)) // <<< Here
                {
                    ...
                    {
                        if (text.Text.Contains(_rep.TextPlaceholderStartTag + replace.Key +
                                               _rep.TextPlaceholderEndTag))
                        {
                            if (!string.IsNullOrEmpty(replace.Value) && replace.Value.Contains(_rep.NewLineTag)
                            ) //If we have line breaks present
                            {
                                string[] repArray = replace.Value.Split(new string[] {_rep.NewLineTag},
                                ...
                                for (var i = 0; i < repArray.Length; i++)
                                {
                                    ...
                                    ) //The text is only replaced with the first part of the replacement array
                                    {
                                        text.Text = text.Text.Replace(
                                            _rep.TextPlaceholderStartTag + replace.Key + _rep.TextPlaceholderEndTag,
                                            repArray[i]);
                ...
                foreach (var trDict in _rep.TablePlaceholders) //Take a Row (one Dictionary) at a time
                {
                    ...
                    var textElements =
                        doc.MainDocumentPart.Document.Body.Descendants<Text>()
                            ...
                            .Where(t =>
                                trDict.Keys.Select(key =>
                                        _rep.TablePlaceholderStartTag + trCol0.Key + _rep.TablePlaceholderEndTag)
                                    .Any(s => t.Text.Contains(s)) &&
                            ...
                            ) //Cycle through the cells of the row to replace from the Dictionary value ( string array)
                            {
                                    ...
                                    index++) //Now cycle through the "columns" (keys) of the Dictionary and replace item by item
                                {
                                                           ...
                                                           _rep.TablePlaceholderEndTag))
                                    {
                                        ...
                                        {
                                            string[] repArray = item.Value[j].Split(new[] {_rep.NewLineTag},
                                                StringSplitOptions.None);
